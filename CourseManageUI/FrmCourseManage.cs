...
namespace CourseManageUI
{
    public partial class FrmCourseManage : Form
    {
        ...
        //创建一个集合，用来缓存查询结果，便于后面修改和删除使用，从而减少数据库的访问
        private List<Course> queryList = null;
        ...
        {
            //【1】条件验证
            if ((this.cbbCategory.SelectedIndex == -1 || this.cbbCategory.SelectedIndex == 0) //用户分类没选或选的"请选择"
                && this.txtCourseName.Text.Trim().Length == 0)  //课程名字没写
            {
            ...
            else  //重新绑定查询结果
            {
                ...
                this.lblCourseCount.Text = this.dgvCourseList.RowCount.ToString();
                //启用修改和删除按钮（提高用户体验）
        ...
        private void btnDelCourse_Click(object sender, EventArgs e)
        {
            ...
            this.lblCourseCount.Text = this.queryList.Count.ToString();
        }

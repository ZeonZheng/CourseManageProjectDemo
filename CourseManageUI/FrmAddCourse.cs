...
namespace CourseManageUI
{
    public partial class FrmAddCourse : Form
    {
        ...
        private void btnSaveToDB_Click(object sender, EventArgs e)
        {
            ...
            //为空的判断请自己完成...
            #region  为空判断作业
            if (this.txtCourseName.Text.Trim().Length == 0)
            {
            ...
            //密码验证
            if (this.txtClassHour.Text.Trim().Length == 0)
            {
                MessageBox.Show("课时总长不能为空");
                //光标定位
                this.txtClassHour.Focus();
                //立即返回，下面的代码不执行
                return;
            }
            if (this.txtCredit.Text.Trim().Length == 0)
            {
                MessageBox.Show("课时学分不能为空");
                //光标定位
                this.txtCredit.Focus();
                //立即返回，下面的代码不执行
                return;
            }
            if (this.txtCourseContent.Text.Trim().Length == 0)
            {
                MessageBox.Show("课时描述不能为空");
                //光标定位
                this.txtCourseContent.Focus();
                //立即返回，下面的代码不执行
                return;
            }
            if (this.cbbCategory.Text.Trim().Length == 0)
            {
                MessageBox.Show("课时描述不能为空");
                //光标定位
                this.cbbCategory.Focus();
                //立即返回，下面的代码不执行
                return;
            }
            #endregion
